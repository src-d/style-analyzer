...
def get_parser() -> argparse.ArgumentParser:
    ...
    create_typos_parser = add_parser("create_typos", "Create random edit typos with given \
                                     probabilities in a dataset of tokens. May also split resulting \
                                     dataset in given proportions")
    ...
    create_typos_parser.add_argument("input_file", metavar="input file", type=str, 
                                     help="File of pickled input pandas dataframe. Dataframe must \
                                     contain column 'identifier'")
    
    create_typos_parser.add_argument("out_file", metavar="output file", type=str,
                                     help="File to dump pickled output dataframe. Dataframe contains \
                                     same columns as input one plus columns \'typo\' and \'corrupted\'")
    ...
    create_typos_parser.add_argument("-s", "--test_portion", type=float, help="Test portion \
                                     for spliting data on train and test chunks without mixing rows")
    ...
    baseline_parser.add_argument("-tr", "--train_file", type=str,
                                 help="File of pickled training dataframe. \
                                 Dataframe indexed by 'id' and containing columns 'identifier', 'typo'")
    ...
    baseline_parser.add_argument("-ctr", "--cand_train_file", type=str,
                                 ...
                                 ('id', 'typo', 'candidate', 'typo_freq', 'cand_freq', 'distance')")
    ...
    filter_parser = add_parser("filter_identifiers", "Filter dataframe of splitted identifiers. \
                               Leave only rows with identifiers, which contain as tokens after split \
                               only words from provided vocabulary collection")
