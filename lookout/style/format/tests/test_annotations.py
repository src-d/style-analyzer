...
class AnnotationsTests(unittest.TestCase):
    def test_annotations(self):
        ...
        annotations = list(annotated_data.iter_items([AtomicTokenAnnotation, PathAnnotation]))
        res = list(zip(("012", "", "3"),
                       (AnnotationsSlice(0, 3, {AtomicTokenAnnotation: token_annotations[0],
                                                PathAnnotation: path_annotation}),
                        AnnotationsSlice(3, 3, {AtomicTokenAnnotation: token_annotations[1],
                                                PathAnnotation: path_annotation}),
                        AnnotationsSlice(3, 4, {AtomicTokenAnnotation: token_annotations[2],
                                                PathAnnotation: path_annotation}))))
