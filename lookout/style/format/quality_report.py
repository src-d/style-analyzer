...
def quality_report(input_pattern: str, bblfsh: str, language: str, n_files: int, model_path: str
    ...
    y_pred, _, safe_preds = rules.predict(X=X, y=y, vnodes_y=vnodes_y, vnodes=vnodes,
                                          files={f.path: f for f in files}, feature_extractor=fe,
                                          client=client, vnodes_parents=vnodes_parents,
                                          parents=parents)
    y = y[safe_preds]
    vnodes_y = [vn for i, vn in enumerate(vnodes_y) if i in safe_preds]
        ...
        for lang, head_files in head_files_by_lang.items():
            ...
            for file in filter_files(head_files, rules.origin_config["line_length_limit"]):
                ...
                if res is None:
                    ...
                    continue
                X, y, vnodes_y, vnodes, vnodes_parents, parents = res
                y_pred, rule_winners, safe_preds = rules.predict(X=X, y=y, vnodes_y=vnodes_y,
                                                                 vnodes=vnodes,
    ...
    def train(cls, ptr: ReferencePointer, config: MutableMapping[str, Any],
        ...
        for lang, head_files in files_by_language(data["files"]).items():
            ...
            y_pred, rule_winners, safe_preds = rules.predict(
                X=X, y=y, vnodes_y=vnodes_y, vnodes=vnodes,
                files={f.path: f for f in filtered_files}, feature_extractor=fe,
                client=cls.client, vnodes_parents=vnodes_parents, parents=parents)
            y = y[safe_preds]
            vnodes_y = [vn for i, vn in enumerate(vnodes_y) if i in safe_preds]
            target_names = get_composite_class_representations(fe)
            assert len(y) == len(y_pred)
