...
class QualityReport:
    ...
    def __init__(self, quality: Optional[str] = None, model: Optional[str] = None,
                 test: Optional[str] = None) -> None:
        """
        Init method.

        :param quality: Quality report string generated by generate_quality_report.
        :param model: Model report string generated by generate_model_report.
        :param test: Test report string generated by generate_test_report.
        """
...
def main(args):
    ...
    with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            for ri, repo in enumerate(REPOSITORIES):
                ...
                try:
                    ...
                    if (report.quality is not None and
                            report.model is not None and
                            report.test is not None):
                        reports.append((repo, report))
                    else:
                        log.warning("skipped %s: quality %s, model %s, test %s", repo,
        ...
        for report_name in ("quality", "test"):
