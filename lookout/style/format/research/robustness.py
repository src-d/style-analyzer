...
def get_content_from_repo(folder: str):
    content = {}
    filenames = glob.glob(folder, recursive=True)
    for file in filter_filepaths(filenames):
        if file.endswith(".js"):
            with open(file, "r") as g:
                #"/".join(file.split("/")[file.split("/").index("jquery"):])
                content[file] = g.read()
                #content.append((file, g.read()))
    return content

    
def get_difflib_changes(true_content, noisy_content):
    true_files, noisy_files = set(), set()
    nb_changes = 0
    lines_changed = defaultdict(set)
    #for i in range(len(true_content)):
    #for i in range(len(noisy_content)):
    for (tf, tc), (nf, nc) in zip(true_content.items(), noisy_content.items()):
        #import pdb;pdb.set_trace()
        #for file in noisy_content.keys():
        matcher = SequenceMatcher(a=tc.splitlines(), b=nc.splitlines())
        #matcher = SequenceMatcher(a=true_content[i][1].splitlines(), b=noisy_content[i][1].splitlines())
...
def get_mispreds(y, y_pred, nodes, winner):
    mispreds = []
...
def get_diff_mispreds(mispreds_noise, lines_changed):
    diff_mispreds = {}
...
def files2mispreds(files, rules, client, language):
...
def style_robustness_report(true_repo: str, noisy_repo: str, bblfsh: str, language: str, model: str):
    ...
    mispreds_true, vnodes_y_true = files2mispreds(true_files, rules, client, language)
    ...
    print("Number of artificial mistakes potentially fixed by the model : %d / %d" % (len(diff_mispreds), nb_changes))
