...
from lookout.style.typos.typos_correction.utils import join_lists
from lookout.style.typos.typos_correction.candidates_generation import (CandidatesGenerator,
                                                                        CorrectionsFinder,
                                                                        get_candidates_features,
                                                                        get_candidates_tokens)
from lookout.style.typos.typos_correction.ranking import CandidatesRanker
...
class TyposCorrector(Model):
    ...
    def __init__(self, emb_file: str, threads_number: int = DEFAULT_THREADS_NUMBER, nn_file=None):
        super().__init__()
        self.generator = CandidatesGenerator(emb_file, threads_number, nn_file)
    ...
    def create_model(self, vocabulary_file: str, frequencies_file: str,
                     ...
                     kdtree_leaf: int = DEFAULT_KDTREE_LEAF,
                     radius: int = DEFAULT_RADIUS) -> None:
        self.finder.construct(vocabulary_file, frequencies_file, self.generator.fasttext,
                              neighbors_number, taken_for_distance, max_distance, kdtree_leaf,
                              radius)

    def _generate_tree(self) -> dict:
        return {"finder": self.finder._generate_tree(),
                "ranker": self.ranker._generate_tree()}

    def _load_tree(self, tree: dict) -> None:
        finder_dict = tree["finder"]
        finder_dict["fasttext"] = self.generator.fasttext
    ...
    def train(self, typos: pandas.DataFrame, candidates: pandas.DataFrame = None,
        ...
        if candidates is None:
            candidates = self.generator.generate_candidates(typos, self.finder,
