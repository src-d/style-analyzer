...
class SymSpell:
    ...
    The Symmetric Delete spelling correction algorithm reduces
    the complexity of edit candidate generation and dictionary lookup
    for a given Damerau-Levenshtein distance. It is six orders
    of magnitude faster and language independent.
    Opposite to other algorithms only deletes are required,
    no transposes + replaces + inserts.
    Transposes + replaces + inserts of the input term are transformed
    into deletes of the dictionary term.
    Replaces and inserts are expensive and language dependent:
    e.g. Chinese has 70,000 Unicode Han characters!

    SymSpell supports compound splitting / decompounding
    of multi-word input strings with three cases:
    1. mistakenly inserted space into a correct word led
    to two incorrect terms
    2. mistakenly omitted space between two correct words led
    ...
    Args:
        max_dictionary_edit_distance (int, optional): Maximum distance
            used to generate index. Also acts
            as an upper bound for `max_edit_distance` parameter
            in `lookup()` method. Defaults to 2.
        ...
        count_threshold (int, optional): Threshold corpus-count
            value for words to be considered correct.
            Defaults to 1, values below zero are also mapped to 1.
            Consider setting a higher value if your
    ...
    def create_dictionary(self, corpus):
        ...
        Note:
            Words are not preprocessed in any way. It is your duty to provide
                appropriate corpus. Also keep in mind that the distance used to
                generate index is specified at initialization. Consider
        ...
        while candidate_ptr < len(candidates):
            ...
            if self._hash(candidate) in self._deletes.keys():
                ...
                for suggestion in dictionary_suggestions:
                    ...
                    if suggestion_prefix_len > phrase_prefix_len and \
                    ...
                    if candidate_len == 0:
                        ...
                        if (self._prefix_length - max_edit_distance == candidate_len and
                                (min_distance > 1 and
                                 phrase[:phrase_len + 1 - min_distance] !=
                                 suggestion[:phrase_len + 1 - min_distance]) or
                                (min_distance > 0 and
                                 phrase[phrase_len - min_distance] !=
                                 suggestion[suggestion_len - min_distance] and
                                 (phrase[phrase_len - min_distance - 1] !=
                                  suggestion[suggestion_len - min_distance] or
                                  phrase[phrase_len - min_distance] !=
                                  suggestion[suggestion_len - min_distance - 1]))):
                            continue
                        else:
                            if (verbosity < 2 and
                                not self._delete_in_suggestion_prefix(candidate,
                                                                      candidate_len,
                                                                      suggestion,
                                                                      suggestion_len) or
                    ...
                    if distance <= max_edit_distance_2:
                        ...
                        if len(suggestions) > 0:
                            if verbosity == 0:  # Top
                                if (distance < max_edit_distance_2 or
    ...
    def lookup_compound(self, phrase, max_edit_distance):
        ...
        for i in range(len(terms_list_1)):
            ...
            if i > 0 and not last_combi:
                suggestions_combi = self.lookup(terms_list_1[i-1] + terms_list_1[i], 0,
                                                max_edit_distance)
            ...
            if len(suggestions) > 0 and (suggestions[0].distance == 0 or
                                         len(terms_list_1[i]) == 1):
